...
export default function ProfileScreen2() {
  ...
  const fetchProfile = async () => {
    const profile = await profileService.fetchProfile((profileID as string) || self?.id || '');
    setProfile(profile);
  };

  const fetchFollowCounts = async () => {
    const followCounts = await profileService.fetchFollowCounts(
  ...
  const fetchStats = async () => {
    const stats = await profileService.fetchStats((profileID as string) || self?.id || '');
    setStats(stats);
  };
